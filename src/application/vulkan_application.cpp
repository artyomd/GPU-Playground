...
application::VulkanApplication::VulkanApplication() : GlfwApplication() {}

void application::VulkanApplication::SetupWindowHints() {
  glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
}

void application::VulkanApplication::OnWindowSizeChanged(int width,
                                                         int height) {
...
void application::VulkanApplication::InitContext() {
  ...
  auto context = std::make_shared<api::vulkan::VulkanRenderingContext>(
      physical_device_, device_, graphics_queue_, graphics_command_pool_,
      swap_chain_image_format_, max_frames_in_flight_);
  SetContext(context);
  CreateImageViews();
...
void application::VulkanApplication::CreateInstance() {
  ...
  instance_create_info.pNext = instance_create_info_next;
  ...
  messenger_create_info.pfnUserCallback =
...
void application::VulkanApplication::CreateSurface() {
  if (glfwCreateWindowSurface(vulkan_instance_, window_, nullptr, &surface_) !=
      VK_SUCCESS) {
...
application::VulkanApplication::FindQueueFamilies(VkPhysicalDevice device) {
...
bool application::VulkanApplication::CheckDeviceExtensionSupport(
    VkPhysicalDevice device) {
...
application::VulkanApplication::SwapChainSupportDetails
application::VulkanApplication::QuerySwapChainSupport(VkPhysicalDevice device) {
...
void application::VulkanApplication::CreateSwapChain() {
  ...
  VkSurfaceFormatKHR surface_format = ChooseSwapSurfaceFormat(
      swap_chain_support.formats, VK_FORMAT_R8G8B8A8_UNORM);
...
VkExtent2D application::VulkanApplication::ChooseSwapExtent(
...
void application::VulkanApplication::CreateImageViews() {
  ...
  for (size_t i = 0; i < swap_chain_images_.size(); i++) {
    GetVulkanContext()->CreateImageView(
        ...
        VK_IMAGE_ASPECT_COLOR_BIT, &swap_chain_image_views_[i]);
  }
}
...
void application::VulkanApplication::CreateColorResources() {
  ...
  context->CreateImage(swap_chain_extent_.width, swap_chain_extent_.height,
                       VK_SAMPLE_COUNT_4_BIT, swap_chain_image_format_,
                       VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT |
                           VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &color_image_,
                       &color_image_memory_);
  context->CreateImageView(color_image_, swap_chain_image_format_,
                           VK_IMAGE_ASPECT_COLOR_BIT, &color_image_view_);
  context->TransitionImageLayout(color_image_, VK_IMAGE_LAYOUT_UNDEFINED,
...
void application::VulkanApplication::CreateDepthResources() {
  ...
  context->CreateImage(swap_chain_extent_.width, swap_chain_extent_.height,
                       VK_SAMPLE_COUNT_4_BIT, depth_format,
                       VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &depth_image_,
                       &depth_image_memory_);
  context->CreateImageView(depth_image_, depth_format,
                           VK_IMAGE_ASPECT_DEPTH_BIT, &depth_image_view_);
...
void application::VulkanApplication::InitImGui() {
  ...
  VkCommandBuffer buffer =
      context->BeginSingleTimeCommands(graphics_command_pool_);
  ImGui_ImplVulkan_CreateFontsTexture(buffer);
  context->EndSingleTimeCommands(graphics_queue_, graphics_command_pool_,
                                 buffer);
...
bool application::VulkanApplication::PrepareFrame() {
  ...
  if (images_in_flight_[current_image_] != VK_NULL_HANDLE) {
    vkWaitForFences(device_, 1, &images_in_flight_[current_image_], VK_TRUE,
                    UINT64_MAX);
  }
  ...
  vkCmdBeginRenderPass(graphics_command_buffers_[current_frame_],
...
void application::VulkanApplication::RenderImGui() {
  ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(),
                                  graphics_command_buffers_[current_frame_]);
}
...
std::shared_ptr<api::vulkan::VulkanRenderingContext>
application::VulkanApplication::GetVulkanContext() {
  return std::dynamic_pointer_cast<api::vulkan::VulkanRenderingContext>(
      GetContext());
}
