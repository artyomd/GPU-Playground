...
void application::GlfwApplication::InitContext() {
  InitWindow();
}
...
void application::GlfwApplication::InitWindow() {
  ...
  window_ = glfwCreateWindow(static_cast<int>(window_width_),
                             ...
                             "GPU Playground", nullptr, nullptr);
  if (window_ == nullptr) {
    getchar();
    glfwTerminate();
...
void application::GlfwApplication::DestroyContext() {
  DestroyWindow();
}

void application::GlfwApplication::OnTestChanged(std::shared_ptr<test::Test> current_test) {
