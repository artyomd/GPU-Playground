...
test::TestSphere::TestSphere(
  ...
  auto sphere = std::make_shared<geometry::SpiralSphere>(rendering_context_,
                                                         1.0F, 32, 64);
  ...
  pipeline_ = rendering_context_->CreateGraphicsPipeline(
      vertex_shader, fragment_shader, sphere->GetVbl(),
      {api::DrawMode::TRIANGLE_STRIP, api::CullMode::NONE, api::FrontFace::CCW,
       true, api::CompareOp::LESS});
  pipeline_->SetVertexBuffer(sphere->GetVertexBuffer());
  pipeline_->SetIndexBuffer(sphere->GetIndexBuffer(),
                            sphere->GetIndexBufferDataType());
