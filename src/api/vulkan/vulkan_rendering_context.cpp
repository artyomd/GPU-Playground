...
api::vulkan::VulkanRenderingContext::VulkanRenderingContext(VkPhysicalDevice physical_device,
                                                            VkDevice device,
  ...
  std::vector<VkDescriptorPoolSize> pool_sizes =
      {{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 50}, {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 50},
...
std::shared_ptr<api::RenderingPipeline> api::vulkan::VulkanRenderingContext::CreateGraphicsPipeline(std::shared_ptr<
    Shader> vertex_shader,
                                                                                                    std::shared_ptr<
                                                                                                        Shader> fragment_shader,
                                                                                                    const api::VertexBufferLayout &vbl,
                                                                                                    api::RenderingPipelineConfig config) {
  return std::make_shared<VulkanRenderingPipeline>(shared_from_this(), vertex_shader, fragment_shader, vbl, config);
}

std::shared_ptr<api::Shader> api::vulkan::VulkanRenderingContext::CreateShader(std::string sipr_v_shader_location,
                                                                               std::string entry_point_name,
                                                                               api::ShaderType type) {
  return std::make_shared<VulkanShader>(shared_from_this(), sipr_v_shader_location, entry_point_name, type);
}
...
VkFormat api::vulkan::VulkanRenderingContext::FindSupportedFormat(const std::vector<VkFormat> &candidates,
