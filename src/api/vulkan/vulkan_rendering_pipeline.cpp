...
api::vulkan::VulkanRenderingPipeline::VulkanRenderingPipeline(
  ...
  this->fragment_shader_ =
      std::dynamic_pointer_cast<VulkanShader>(fragment_shader);
  CreatePipeline(vbl);
}

void api::vulkan::VulkanRenderingPipeline::SetVertexBuffer(
    std::shared_ptr<Buffer> buffer) {
  this->vertex_buffer_ = std::dynamic_pointer_cast<VulkanBuffer>(buffer);
}

void api::vulkan::VulkanRenderingPipeline::SetIndexBuffer(
...
void api::vulkan::VulkanRenderingPipeline::CreatePipeline(
  ...
  pipeline_bindings.reserve(pipeline_bindings.size() +
                            fragment_bindings.size());
  pipeline_bindings.insert(pipeline_bindings.end(), fragment_bindings.begin(),
  ...
  descriptor_set_layout_create_info.bindingCount =
      static_cast<uint32_t>(pipeline_bindings.size());
  ...
  for (auto element : elements) {
    ...
    description.format =
        GetVkFormat(element.type, static_cast<uint32_t>(element.count));
...
void api::vulkan::VulkanRenderingPipeline::CreateUniformBuffers(
  ...
  for (auto binding : shader->GetBindings()) {
    if (binding.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER &&
      ...
      for (uint32_t i = 0; i < context_->GetImageCount(); i++) {
        auto vulkan_buffer = std::make_shared<VulkanBuffer>(
            context_, size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
void api::vulkan::VulkanRenderingPipeline::Draw(size_t index_count,
                                                size_t offset) {
  ...
  vkCmdBindIndexBuffer(command_buffer, index_buffer_->GetBuffer(), offset,
                       this->index_type_);
  vkCmdBindDescriptorSets(
      command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout_, 0, 1,
      &descriptor_sets_[context_->GetCurrentImageIndex()], 0, nullptr);
  vkCmdDrawIndexed(context_->GetCurrentCommandBuffer(),
                   static_cast<uint32_t>(index_count), 1, 0, 0, 0);
}

void api::vulkan::VulkanRenderingPipeline::UpdateUniformBuffer(
...
void api::vulkan::VulkanRenderingPipeline::SetTexture(
  ...
  for (auto binding : fragment_shader_->GetBindings()) {
    if (binding.binding == binding_point &&
  ...
  if (!binding_available) {
    spdlog::warn("warning: entry point {} does not exists in shaders",
                 binding_point);
    return;
  }
  ...
  for (uint32_t i = 0; i < context_->GetImageCount(); i++) {
    auto descriptor_write =
        vk_uniform->GetWriteDescriptorSetFor(i, binding_point);
...
void api::vulkan::VulkanRenderingPipeline::SetViewPort(uint32_t width,
                                                       uint32_t height) {
...
api::vulkan::VulkanRenderingPipeline::~VulkanRenderingPipeline() {
  ...
  vkFreeDescriptorSets(device_, context_->GetDescriptorPool(),
