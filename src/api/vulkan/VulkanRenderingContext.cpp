...
#include "src/api/vulkan/VulkanIndexBuffer.h"
...
#include "src/api/vulkan/VulkanUniformBuffer.h"
#include "src/api/vulkan/VulkanVertexBuffer.h"
...
VkSampleCountFlagBits VulkanRenderingContext::GetMaxUsableSampleCount() {
...
std::shared_ptr<IndexBuffer> VulkanRenderingContext::CreateIndexBuffer(const void *data,
                                                                       unsigned int size,
                                                                       DataType type) {
  return std::make_shared<VulkanIndexBuffer>(shared_from_this(), data, size, type);
}

std::shared_ptr<VertexBuffer> VulkanRenderingContext::CreateVertexBuffer(const void *data, unsigned int size) {
...
VkRenderPass VulkanRenderingContext::GetVkRenderPass() const {
  return vk_render_pass_;
}
VkExtent2D VulkanRenderingContext::GetSwapChainExtent() const {
  return swap_chain_extent_;
}
void VulkanRenderingContext::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
...
void VulkanRenderingContext::TransitionImageLayout(VkImage image,
...
void VulkanRenderingContext::CreateBuffer(VkDeviceSize size,
...
VkDescriptorPool VulkanRenderingContext::GetDescriptorPool() {
  return descriptor_pool_;
}
int VulkanRenderingContext::GetImageCount() const {
  return image_count_;
}

void VulkanRenderingContext::CopyBuffer(VkBuffer src_buffer, VkBuffer dst_buffer, VkDeviceSize size) {
  VkCommandBuffer command_buffer = BeginSingleTimeCommands(graphics_pool_);
...
uint32_t VulkanRenderingContext::FindMemoryType(uint32_t type_filter, VkMemoryPropertyFlags properties) const {
...
void VulkanRenderingContext::CreateImageView(VkImage image,
                                             VkFormat format,
...
VkCommandBuffer VulkanRenderingContext::BeginSingleTimeCommands(VkCommandPool command_pool) {
...
void
VulkanRenderingContext::EndSingleTimeCommands(VkQueue queue, VkCommandPool pool, VkCommandBuffer command_buffer) {
...
std::shared_ptr<VertexBinding> VulkanRenderingContext::CreateVertexBinding(
...
std::shared_ptr<RenderingPipeline> VulkanRenderingContext::CreateGraphicsPipeline(
...
std::shared_ptr<Shader> VulkanRenderingContext::CreateShader(std::string sipr_v_shader_location,
...
VkCommandBuffer VulkanRenderingContext::GetCurrentCommandBuffer() const {
  return current_command_buffer_;
}

void VulkanRenderingContext::SetCurrentCommandBuffer(VkCommandBuffer current_command_buffer) {
  current_command_buffer_ = current_command_buffer;
}

void VulkanRenderingContext::SetVkRenderPass(VkRenderPass vk_render_pass) {
  vk_render_pass_ = vk_render_pass;
}

std::shared_ptr<UniformBuffer> VulkanRenderingContext::CreateUniformBuffer(
    int length,
    int binding_point,
...
std::shared_ptr<Texture2D> VulkanRenderingContext::CreateTexture2D(std::string image_path,
                                                                   int binding_point,
                                                                   ShaderType shader_stage) {
  return std::make_shared<VulkanTexture2D>(shared_from_this(), image_path, binding_point, shader_stage);
}

int VulkanRenderingContext::GetCurrentImageIndex() const {
  return current_image_index_;
}

void VulkanRenderingContext::SetCurrentImageIndex(int current_image_index) {
  current_image_index_ = current_image_index;
}

void VulkanRenderingContext::SetViewportSize(int width, int height) {
...
void VulkanRenderingContext::WaitForGpuIdle() const {
  vkDeviceWaitIdle(device_);
}

std::shared_ptr<RenderingPipelineLayout> VulkanRenderingContext::CreateRenderingPipelineLayout(
...
VkFormat VulkanRenderingContext::FindDepthFormat() const {
...
VkFormat VulkanRenderingContext::FindSupportedFormat(
...
void VulkanRenderingContext::CreateImage(uint32_t width,
                                         uint32_t height,
...
VkSampleCountFlagBits VulkanRenderingContext::GetMsaaSamples() const {
