...
api::VkRenderingPipeline::VkRenderingPipeline(VkRenderingContext *context,
  ...
  auto binding_description = dynamic_cast<const VkVertexBinding *>(vertex_binding)->GetVertexInputBindingDescription();
  ...
  viewport.
      y = 0.0f;
  viewport.
      width = (float) context->GetSwapChainExtent()->width;
  viewport.
      height = (float) context->GetSwapChainExtent()->height;
  viewport.
      minDepth = 0.0f;
  viewport.
      maxDepth = 1.0f;
  ...
  pipeline_info.
      renderPass = *context->GetVkRenderPass();
  pipeline_info.
      subpass = 0;
  pipeline_info.
