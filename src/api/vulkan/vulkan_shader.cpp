...
api::vulkan::VulkanShader::VulkanShader(
  ...
  SpvReflectResult result = spvReflectCreateShaderModule(
      code_.size(), code_.data(), &reflect_shader_module_);

  uint32_t count = 0;
  result = spvReflectEnumerateEntryPointDescriptorSets(
      &reflect_shader_module_, this->entry_point_name_.data(), &count, nullptr);
  ...
  result = spvReflectEnumerateEntryPointDescriptorSets(
      &reflect_shader_module_, this->entry_point_name_.data(), &count,
      sets.data());
  CHECK(result == SPV_REFLECT_RESULT_SUCCESS, "spir-v reflection failed");
  ...
  if (count == 1) {
    ...
    for (uint32_t i_binding = 0; i_binding < set->binding_count; ++i_binding) {
      const SpvReflectDescriptorBinding &reflect_descriptor_binding =
          *(set->bindings[i_binding]);
      ...
      for (uint32_t i_dim = 0;
           i_dim < reflect_descriptor_binding.array.dims_count; ++i_dim) {
        layout_binding.descriptorCount *=
            reflect_descriptor_binding.array.dims[i_dim];
      }
...
VkPipelineShaderStageCreateInfo api::vulkan::VulkanShader::GetShaderStageInfo()
    const {
  if (constants_changed_) {
    shader_specialization_info_.mapEntryCount =
  ...
  [&](data_type v) {                                                       \
    ...
    *(reinterpret_cast<data_type *>(static_cast<char *>(spec_data_) +      \
    ...
    specialization_map_entry.size = sizeof(data_type);                     \
                   ...
                   VISIT(bool), VISIT(unsigned int), VISIT(int), VISIT(float),
                   VISIT(double));
      specialization_map_entries_.emplace_back(specialization_map_entry);
    }
    shader_specialization_info_.pMapEntries =
        specialization_map_entries_.data();
...
api::vulkan::VulkanShader::GetBindings() const {
  return bindings_;
}

size_t api::vulkan::VulkanShader::DescriptorSizeInBytes(
    unsigned int binding_point) const {
  uint32_t count = 1;
  std::vector<SpvReflectDescriptorSet *> sets(count);
  auto result = spvReflectEnumerateDescriptorSets(&reflect_shader_module_,
                                                  &count, sets.data());
  CHECK(result == SPV_REFLECT_RESULT_SUCCESS, "spir-v reflection failed");
  for (uint32_t i_binding = 0; i_binding < sets[0]->binding_count;
       ++i_binding) {
    const SpvReflectDescriptorBinding &reflect_descriptor_binding =
        *(sets[0]->bindings[i_binding]);
