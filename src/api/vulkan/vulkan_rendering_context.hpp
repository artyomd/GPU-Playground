...
class VulkanRenderingContext
 ...
 public:
  ...
  std::shared_ptr<Buffer> CreateBuffer(size_t size_in_bytes, BufferUsage usage,
                                       MemoryType memory_type) override;

  std::shared_ptr<api::RenderingPipeline> CreateGraphicsPipeline(
  ...
  void CopyBufferToImage(VkBuffer buffer, VkImage image, size_t width,
                         size_t height);
  ...
  void EndSingleTimeCommands(VkQueue queue, VkCommandPool pool,
