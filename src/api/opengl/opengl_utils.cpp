...
bool api::opengl::GlLogCall(const char *function, const char *file, int line) {
  ...
  if (UTILS_UNLIKELY(error != GL_NO_ERROR)) {
    spdlog::error("[OpenGL error] ({:x}): {} {}:{}", error, function, file,
                  line);
    return false;
  }
  return true;
...
GLint api::opengl::GetShaderGlType(api::ShaderType shader_type) {
  switch (shader_type) {
    case api::ShaderType::VERTEX:
      return GL_VERTEX_SHADER;
    case api::ShaderType::FRAGMENT:
...
GLint api::opengl::GetGlAddressMode(api::AddressMode address_mode) {
  switch (address_mode) {
    case api::AddressMode::REPEAT:
      return GL_REPEAT;
    case api::AddressMode::MIRRORED_REPEAT:
      return GL_MIRRORED_REPEAT;
    case api::AddressMode::CLAMP_TO_EDGE:
      return GL_CLAMP_TO_EDGE;
    case api::AddressMode::CLAMP_TO_BORDER:
