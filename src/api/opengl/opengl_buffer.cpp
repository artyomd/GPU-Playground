...
api::opengl::OpenGlBuffer::OpenGlBuffer(const size_t &length) : Buffer(length) {
  ...
  GL_CALL(
      glBufferData(GL_ARRAY_BUFFER, size_in_bytes_, nullptr, GL_STATIC_DRAW));
...
void api::opengl::OpenGlBuffer::CopyFrom(
    std::shared_ptr<api::Buffer> src_buffer, size_t size, size_t src_offset,
    size_t dst_offset) {
