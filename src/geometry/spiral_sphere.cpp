...
geometry::SpiralSphere::SpiralSphere(const std::shared_ptr<vulkan::RenderingContext> &context, const float radius,
                                     const unsigned int loops, const unsigned int segments_per_loop)
  ...
  for (unsigned int loop_segment_number = 0; loop_segment_number < segments_per_loop; ++loop_segment_number) {
    ...
    const auto phi = static_cast<float>(loop_segment_number * 2 * M_PI / segments_per_loop);
  ...
  for (unsigned int loop_number = 0; loop_number <= loops; ++loop_number) {
    for (unsigned int loop_segment_number = 0; loop_segment_number < segments_per_loop; ++loop_segment_number) {
      ...
      const float cos_phi = std::cos(phi);
      PointWithColor point = {
          radius * cos_phi * sin_theta, radius * sin_phi * sin_theta, radius * cos_theta, 255, 255, 255, 255};
      geometry_data.push_back(point);
    }
  }
  for (unsigned int loop_segment_number = 0; loop_segment_number < segments_per_loop; ++loop_segment_number) {
