...
struct Vertex {
  glm::vec3 position;
  glm::vec2 tex_coord;
  bool operator==(const Vertex &other) const { return position == other.position && tex_coord == other.tex_coord; }
};
}  // namespace
...
std::shared_ptr<renderable::ObjModel> renderable::ObjModel::Create(
...
renderable::ObjModel::ObjModel(const std::shared_ptr<vulkan::RenderingContext> &context,
  ...
  std::string warn;
  std::string err;
  if (!LoadObj(&attrib, &shapes, &materials, &warn, &err, (RESOURCE_DIR + std::string("/models/chalet.obj")).c_str(),
  ...
  for (const auto &kShape : shapes) {
    for (const auto &kIndex : kShape.mesh.indices) {
      Vertex vertex = {};
      vertex.position = {attrib.vertices[3 * static_cast<uint64_t>(kIndex.vertex_index) + 0],
                         attrib.vertices[3 * static_cast<uint64_t>(kIndex.vertex_index) + 1],
                         attrib.vertices[3 * static_cast<uint64_t>(kIndex.vertex_index) + 2]};
      vertex.tex_coord = {attrib.texcoords[2 * static_cast<uint64_t>(kIndex.texcoord_index) + 0],
...
#include "texture2d_fragment.spv"
  ...
  int tex_height = 0;
  int tex_channels = 0;
  stbi_uc *pixels = stbi_load((RESOURCE_DIR + std::string("/textures/chalet.jpg")).c_str(), &tex_width, &tex_height,
  ...
  {
    auto staging_buffer =
        vulkan::Buffer::Create(context, image->GetSizeInBytes(), VK_IMAGE_USAGE_TRANSFER_SRC_BIT, true);
...
std::shared_ptr<vulkan::RenderingPipeline> renderable::ObjModel::CreatePipeline(
...
size_t renderable::ObjModel::NumOfIndicesToDraw() { return index_count_; }
glm::mat4 renderable::ObjModel::ComputeViewMatrix() {
  return lookAt(glm::vec3(-2.0F, 2.0F, 2.0F), glm::vec3(0.0F, 0.0F, 0.0F), glm::vec3(0.0F, 0.0F, 1.0F));
}
renderable::ObjModel::~ObjModel() { WaitForCommandBuffersToFinish(); }
