...
std::shared_ptr<renderable::GltfModel> renderable::GltfModel::Create(std::shared_ptr<vulkan::RenderingContext> context,
                                                                     std::shared_ptr<Menu> parent) {
  return std::shared_ptr<GltfModel>(new GltfModel(context, parent));
}

renderable::GltfModel::GltfModel(std::shared_ptr<vulkan::RenderingContext> context,
                                 std::shared_ptr<Menu> parent) :
...
void renderable::GltfModel::SetupImages(std::vector<std::shared_ptr<vulkan::Image>> images) {
  ...
  for (const auto &kImage : images) {
    ...
    command_buffers_[kImage] = {semaphore, fence, command_buffer, descriptor_index++};
  }
}
...
VkSemaphore renderable::GltfModel::Render(std::shared_ptr<vulkan::Image> image,
  ...
  std::array<VkClearValue, 2> clear_values = {clear_color_value, depth_clear_value};
