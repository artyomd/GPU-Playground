...
std::shared_ptr<vulkan::Framebuffer> vulkan::Framebuffer::Create(std::shared_ptr<RenderingContext> rendering_context,
                                                                 std::shared_ptr<RenderPass> render_pass,
                                                                 std::vector<std::shared_ptr<ImageView>> attachments) {
  auto frameBuffer = new vulkan::Framebuffer(rendering_context, render_pass, attachments);
  return std::shared_ptr<Framebuffer>(frameBuffer);
}

vulkan::Framebuffer::Framebuffer(std::shared_ptr<RenderingContext> rendering_context,
                                 std::shared_ptr<RenderPass> render_pass,
                                 std::vector<std::shared_ptr<ImageView>> attachments) :
...
uint32_t vulkan::Framebuffer::GetWidth() const {
  return attachments_[0]->GetWidth();
}
uint32_t vulkan::Framebuffer::GetHeight() const {
  return attachments_[0]->GetHeight();
}
