...
// Cheap shadows are hard. In fact, I'd almost say, shadowing repeat objects -
// in a setting like this - with limited iterations is impossible... However,
// I'd be very grateful if someone could prove me wrong. :)
float softShadow(vec3 ro, vec3 lp, float k) {
  // More would be nicer. More is always nicer, but not really affordable... Not
  // on my slow test machine, anyway.
  ...
  // Max shadow iterations - More iterations make nicer shadows, but slow things
  // down. Obviously, the lowest number to give a decent shadow is the best one
  // to choose.
  for (int i = 0; i < maxIterationsShad; i++) {
    ...
    shade = min(
        ...
        smoothstep(
            0., 1.,
            k * h / dist));  // Subtle difference. Thanks to IQ for this tidbit.
    // So many options here, and none are perfect: dist += min(h, .2), dist +=
    // clamp(h, .01, .2), clamp(h, .02, stepDist*2.), etc.
    dist += clamp(h, .02, .25);

    // Early exits from accumulative distance function calls tend to be a good
    // thing.
    if (h < 0. || dist > end) break;
    // if (h<.001 || dist > end) break; // If you're prepared to put up with
    // more artifacts.
  }
...
but more symmetrical. Due to
// the intricacies of this particular scene, it's kind of needed to reduce
jagged effects. vec3 getNormal(in vec3 p) { const vec2 e = vec2(.002, 0); return
normalize(vec3(map(p + e.xyy) - map(p - e.xyy), map(p + e.yxy) - map(p - e.yxy),
...
vec3 doColor(in vec3 sp, in vec3 rd, in vec3 sn, in vec3 lp, float t) {
  ...
  vec3 sceneCol =
      (objCol * (diff + .15) + vec3(1., .6, .2) * spec * 2.) * atten;
...
void main() {
  ...
  // the ray origin to describe the surface hit point is kind of confusing. The
  // reason we do it is because the reflective ray will begin from the hit point
  // in the direction of the reflected ray. Thus the new ray origin will be the
  // hit point. See "traceRef" below.
  vec3 sceneColor = doColor(ro, rd, sn, lp, t);

  // Checking to see if the surface is in shadow. Ideally, you'd also check to
  // see if the reflected surface is in shadow. However, shadows are expensive,
  // so it's only performed on the first pass. If you pause and check the
  // reflections, you'll see that they're not shadowed. OMG! - Better call the
  // shadow police. :)
  ...
  // hit point, or in a few cases, beyond the far plane. By the way, for the
  // sake of simplicity, we'll perform a reflective pass for non hit points too.
  // Kind of wasteful, but not really noticeable. The direction of the new ray
  // will obviously be in the direction of the reflected ray. See just above.
  //
  ...
  // Coloring the reflected hit point, then adding a portion of it to the final
  // scene color. How much you add, and how you apply it is up to you, but I'm
  // simply adding 35 percent.
  ...
  // APPLYING SHADOWS
  //
  // Multiply the shadow from the first pass by the final scene color. Ideally,
  // you'd check to see if the reflected point was in shadow, and incorporate
  // that too, but we're cheating to save cycles and skipping it. It's not
  // really noticeable anyway. By the way, ambient occlusion would make it a
  // little nicer, but we're saving cycles and keeping things simple.
  sceneColor *= sh;

  // Clamping the scene color, performing some rough gamma correction (the
  // "sqrt" bit), then presenting it to the screen.
